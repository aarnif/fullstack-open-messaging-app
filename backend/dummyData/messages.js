import users from "./users.js";

const messages = [
  [
    {
      sender: users[0],
      content: "Hey everyone, are we still on for hiking this weekend?",
      createdAt: new Date("2024-06-01T10:00:00Z"),
    },
    {
      sender: users[1],
      content: "Yes! I'm really looking forward to it. What's the plan?",
      createdAt: new Date("2024-06-01T10:05:00Z"),
    },
    {
      sender: users[7],
      content: "Count me in! Should we meet at the usual spot?",
      createdAt: new Date("2024-06-01T10:10:00Z"),
    },
    {
      sender: users[0],
      content:
        "Yes, let's meet at the trailhead at 8 AM. Does that work for everyone?",
      createdAt: new Date("2024-06-01T10:15:00Z"),
    },
    {
      sender: users[1],
      content: "8 AM works for me. What about snacks and water?",
      createdAt: new Date("2024-06-01T10:20:00Z"),
    },
    {
      sender: users[7],
      content: "I'll bring some granola bars and extra water bottles.",
      createdAt: new Date("2024-06-01T10:25:00Z"),
    },
    {
      sender: users[0],
      content:
        "Great, I'll bring some fruits and a first-aid kit just in case.",
      createdAt: new Date("2024-06-01T10:30:00Z"),
    },
    {
      sender: users[1],
      content:
        "Sounds good! Let's make sure to take some breaks and enjoy the view.",
      createdAt: new Date("2024-06-01T10:35:00Z"),
    },
    {
      sender: users[7],
      content: "Absolutely! Looking forward to it. See you all tomorrow!",
      createdAt: new Date("2024-06-01T10:40:00Z"),
    },
    {
      sender: users[0],
      content: "See you all tomorrow! Get a good night's rest.",
      createdAt: new Date("2024-06-01T10:45:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Hey everyone, what book should we read next?",
      createdAt: new Date("2024-06-01T09:00:00Z"),
    },
    {
      sender: users[2],
      content:
        "How about 'To Kill a Mockingbird'? I've heard great things about it.",
      createdAt: new Date("2024-06-01T09:05:00Z"),
    },
    {
      sender: users[3],
      content: "That's a great choice! I've been meaning to read it.",
      createdAt: new Date("2024-06-01T09:10:00Z"),
    },
    {
      sender: users[0],
      content: "I'm in! When should we meet to discuss it?",
      createdAt: new Date("2024-06-01T09:15:00Z"),
    },
    {
      sender: users[2],
      content: "How about next Saturday at 3 PM?",
      createdAt: new Date("2024-06-01T09:20:00Z"),
    },
    {
      sender: users[3],
      content: "That works for me. Should we meet at the usual place?",
      createdAt: new Date("2024-06-01T09:25:00Z"),
    },
    {
      sender: users[0],
      content: "Yes, the coffee shop sounds perfect.",
      createdAt: new Date("2024-06-01T09:30:00Z"),
    },
    {
      sender: users[2],
      content: "I'll bring some snacks!",
      createdAt: new Date("2024-06-01T09:35:00Z"),
    },
    {
      sender: users[3],
      content: "Awesome! I'll see you all next Saturday then.",
      createdAt: new Date("2024-06-01T09:40:00Z"),
    },
    {
      sender: users[0],
      content: "Can't wait! Happy reading, everyone.",
      createdAt: new Date("2024-06-01T09:45:00Z"),
    },
    {
      sender: users[2],
      content: "Hey guys, how far have you gotten with the book?",
      createdAt: new Date("2024-06-02T09:00:00Z"),
    },
    {
      sender: users[3],
      content: "I'm halfway through. It's really good!",
      createdAt: new Date("2024-06-02T09:05:00Z"),
    },
    {
      sender: users[0],
      content: "I just started, but I'm loving it so far.",
      createdAt: new Date("2024-06-02T09:10:00Z"),
    },
    {
      sender: users[2],
      content: "I'm almost done. Can't wait to discuss it.",
      createdAt: new Date("2024-06-02T09:15:00Z"),
    },
    {
      sender: users[3],
      content: "Great! Let's keep going and we'll have a lot to talk about.",
      createdAt: new Date("2024-06-02T09:20:00Z"),
    },
    {
      sender: users[0],
      content: "Agreed! See you all on Saturday.",
      createdAt: new Date("2024-06-02T09:25:00Z"),
    },
    {
      sender: users[2],
      content: "Just finished the book! It's amazing.",
      createdAt: new Date("2024-06-02T09:30:00Z"),
    },
    {
      sender: users[3],
      content: "Same here! It's such a powerful story.",
      createdAt: new Date("2024-06-02T09:35:00Z"),
    },
    {
      sender: users[0],
      content: "I'm so excited for our discussion.",
      createdAt: new Date("2024-06-02T09:40:00Z"),
    },
    {
      sender: users[2],
      content: "Me too! See you all on Saturday!",
      createdAt: new Date("2024-06-02T09:45:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Hey everyone, ready for Gaming Night?",
      createdAt: new Date("2024-06-01T19:00:00Z"),
    },
    {
      sender: users[4],
      content: "Absolutely! What game are we playing tonight?",
      createdAt: new Date("2024-06-01T19:05:00Z"),
    },
    {
      sender: users[5],
      content: "I'm up for some Rocket League. How about you guys?",
      createdAt: new Date("2024-06-01T19:10:00Z"),
    },
    {
      sender: users[1],
      content: "Rocket League sounds fun! I'm in.",
      createdAt: new Date("2024-06-01T19:15:00Z"),
    },
    {
      sender: users[0],
      content: "Great! Let's meet online at 8 PM then.",
      createdAt: new Date("2024-06-01T19:20:00Z"),
    },
    {
      sender: users[4],
      content: "8 PM works for me. I'll bring snacks!",
      createdAt: new Date("2024-06-01T19:25:00Z"),
    },
    {
      sender: users[5],
      content:
        "Snacks sound great! I'll make sure my internet is working well.",
      createdAt: new Date("2024-06-01T19:30:00Z"),
    },
    {
      sender: users[1],
      content: "Looking forward to it. See you all at 8!",
      createdAt: new Date("2024-06-01T19:35:00Z"),
    },
    {
      sender: users[0],
      content: "Hey everyone, I'm online now. Ready when you are!",
      createdAt: new Date("2024-06-01T20:00:00Z"),
    },
    {
      sender: users[4],
      content: "Joining now! Let's get this started.",
      createdAt: new Date("2024-06-01T20:05:00Z"),
    },
    {
      sender: users[5],
      content: "I'm in too. Let's go team!",
      createdAt: new Date("2024-06-01T20:10:00Z"),
    },
    {
      sender: users[1],
      content: "Ready to play! Let's do this.",
      createdAt: new Date("2024-06-01T20:15:00Z"),
    },
    {
      sender: users[0],
      content: "Great game, guys! That was intense.",
      createdAt: new Date("2024-06-01T21:00:00Z"),
    },
    {
      sender: users[4],
      content: "Yeah, we really nailed that last round!",
      createdAt: new Date("2024-06-01T21:05:00Z"),
    },
    {
      sender: users[5],
      content: "Can't believe we pulled off that comeback.",
      createdAt: new Date("2024-06-01T21:10:00Z"),
    },
    {
      sender: users[1],
      content: "Teamwork makes the dream work!",
      createdAt: new Date("2024-06-01T21:15:00Z"),
    },
    {
      sender: users[0],
      content: "We should do this more often.",
      createdAt: new Date("2024-06-01T21:20:00Z"),
    },
    {
      sender: users[4],
      content: "Definitely. How about same time next week?",
      createdAt: new Date("2024-06-01T21:25:00Z"),
    },
    {
      sender: users[5],
      content: "Count me in. This was a blast!",
      createdAt: new Date("2024-06-01T21:30:00Z"),
    },
    {
      sender: users[1],
      content: "I'm up for it. See you guys next week!",
      createdAt: new Date("2024-06-01T21:35:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Good morning, everyone! Ready for our yoga session today?",
      createdAt: new Date("2024-06-01T07:00:00Z"),
    },
    {
      sender: users[3],
      content:
        "Good morning! I'm excited for today's session. What's the focus?",
      createdAt: new Date("2024-06-01T07:05:00Z"),
    },
    {
      sender: users[2],
      content: "Morning! How about we focus on flexibility today?",
      createdAt: new Date("2024-06-01T07:10:00Z"),
    },
    {
      sender: users[8],
      content:
        "Sounds great to me. I've been wanting to improve my flexibility.",
      createdAt: new Date("2024-06-01T07:15:00Z"),
    },
    {
      sender: users[0],
      content: "Awesome! Let's start with some gentle stretches to warm up.",
      createdAt: new Date("2024-06-01T07:20:00Z"),
    },
    {
      sender: users[3],
      content: "Great idea. I'll follow your lead.",
      createdAt: new Date("2024-06-01T07:25:00Z"),
    },
    {
      sender: users[2],
      content: "Same here. Let's get started!",
      createdAt: new Date("2024-06-01T07:30:00Z"),
    },
    {
      sender: users[8],
      content: "Feeling good already. Thanks for the guidance.",
      createdAt: new Date("2024-06-01T07:35:00Z"),
    },
    {
      sender: users[0],
      content:
        "Let's move on to some deeper stretches. How's everyone feeling?",
      createdAt: new Date("2024-06-01T07:40:00Z"),
    },
    {
      sender: users[3],
      content: "Feeling great! Loving the session so far.",
      createdAt: new Date("2024-06-01T07:45:00Z"),
    },
    {
      sender: users[2],
      content: "Feeling the stretch! It's challenging but rewarding.",
      createdAt: new Date("2024-06-01T07:50:00Z"),
    },
    {
      sender: users[8],
      content: "This is fantastic. Thanks for organizing this.",
      createdAt: new Date("2024-06-01T07:55:00Z"),
    },
    {
      sender: users[0],
      content: "You're welcome! Let's try some balance poses next.",
      createdAt: new Date("2024-06-01T08:00:00Z"),
    },
    {
      sender: users[3],
      content: "Balance poses are my favorite. Let's do it.",
      createdAt: new Date("2024-06-01T08:05:00Z"),
    },
    {
      sender: users[2],
      content: "I could use some help with balance. Any tips?",
      createdAt: new Date("2024-06-01T08:10:00Z"),
    },
    {
      sender: users[8],
      content: "Focus on a fixed point and breathe steadily. It helps!",
      createdAt: new Date("2024-06-01T08:15:00Z"),
    },
    {
      sender: users[0],
      content: "Exactly. Let's start with the tree pose.",
      createdAt: new Date("2024-06-01T08:20:00Z"),
    },
    {
      sender: users[3],
      content: "Tree pose is a great choice. Feeling grounded.",
      createdAt: new Date("2024-06-01T08:25:00Z"),
    },
    {
      sender: users[2],
      content: "Thanks for the tip. Feeling more stable already.",
      createdAt: new Date("2024-06-01T08:30:00Z"),
    },
    {
      sender: users[8],
      content: "This is really helpful. Thanks, everyone!",
      createdAt: new Date("2024-06-01T08:35:00Z"),
    },
    {
      sender: users[0],
      content: "Let's finish with some deep breathing and relaxation.",
      createdAt: new Date("2024-06-01T08:40:00Z"),
    },
    {
      sender: users[3],
      content: "Perfect way to end the session. Thanks for leading.",
      createdAt: new Date("2024-06-01T08:45:00Z"),
    },
    {
      sender: users[2],
      content: "Feeling relaxed and refreshed. Great session!",
      createdAt: new Date("2024-06-01T08:50:00Z"),
    },
    {
      sender: users[8],
      content: "Thanks, everyone! Looking forward to the next one.",
      createdAt: new Date("2024-06-01T08:55:00Z"),
    },
    {
      sender: users[0],
      content: "Great job today! See you all next week.",
      createdAt: new Date("2024-06-01T09:00:00Z"),
    },
    {
      sender: users[3],
      content: "Bye everyone! Have a great week.",
      createdAt: new Date("2024-06-01T09:05:00Z"),
    },
    {
      sender: users[2],
      content: "Take care, everyone!",
      createdAt: new Date("2024-06-01T09:10:00Z"),
    },
    {
      sender: users[8],
      content: "See you next time!",
      createdAt: new Date("2024-06-01T09:15:00Z"),
    },
    {
      sender: users[0],
      content: "Remember to practice your balance poses during the week.",
      createdAt: new Date("2024-06-01T09:20:00Z"),
    },
    {
      sender: users[3],
      content: "Will do! Thanks for the reminder.",
      createdAt: new Date("2024-06-01T09:25:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Hey everyone, ready for band practice?",
      createdAt: new Date("2024-06-01T18:00:00Z"),
    },
    {
      sender: users[4],
      content: "Yes! I have a new song idea to share.",
      createdAt: new Date("2024-06-01T18:05:00Z"),
    },
    {
      sender: users[9],
      content: "Can't wait to hear it! Let's start with a warm-up.",
      createdAt: new Date("2024-06-01T18:10:00Z"),
    },
    {
      sender: users[0],
      content: "Good idea. Let's play our usual warm-up tune.",
      createdAt: new Date("2024-06-01T18:15:00Z"),
    },
    {
      sender: users[4],
      content: "Sounds good. Ready when you are!",
      createdAt: new Date("2024-06-01T18:20:00Z"),
    },
    {
      sender: users[9],
      content: "Let's do this!",
      createdAt: new Date("2024-06-01T18:25:00Z"),
    },
    {
      sender: users[0],
      content: "Great warm-up. Now, let's hear that new song idea.",
      createdAt: new Date("2024-06-01T18:30:00Z"),
    },
    {
      sender: users[4],
      content: "Here it is. I was thinking of a mix of rock and jazz.",
      createdAt: new Date("2024-06-01T18:35:00Z"),
    },
    {
      sender: users[9],
      content: "That's interesting. Let's give it a try.",
      createdAt: new Date("2024-06-01T18:40:00Z"),
    },
    {
      sender: users[0],
      content: "I like it! Let's work on the lyrics next.",
      createdAt: new Date("2024-06-01T18:45:00Z"),
    },
    {
      sender: users[4],
      content: "Sure, I have a few lines written down. Let me share them.",
      createdAt: new Date("2024-06-01T18:50:00Z"),
    },
    {
      sender: users[9],
      content: "These lyrics are great. Let's fit them into the melody.",
      createdAt: new Date("2024-06-01T18:55:00Z"),
    },
    {
      sender: users[0],
      content: "Perfect. This song is coming together nicely.",
      createdAt: new Date("2024-06-01T19:00:00Z"),
    },
    {
      sender: users[4],
      content: "Glad you like it. Let's practice it a few more times.",
      createdAt: new Date("2024-06-01T19:05:00Z"),
    },
    {
      sender: users[9],
      content: "Sounds good to me. Practice makes perfect!",
      createdAt: new Date("2024-06-01T19:10:00Z"),
    },
    {
      sender: users[0],
      content: "Agreed. This song is going to be a hit.",
      createdAt: new Date("2024-06-01T19:15:00Z"),
    },
    {
      sender: users[4],
      content: "Definitely. Let's record a demo next week.",
      createdAt: new Date("2024-06-01T19:20:00Z"),
    },
    {
      sender: users[9],
      content: "Can't wait for that. It's going to be awesome.",
      createdAt: new Date("2024-06-01T19:25:00Z"),
    },
    {
      sender: users[0],
      content: "Before we wrap up, should we plan our next gig?",
      createdAt: new Date("2024-06-01T19:30:00Z"),
    },
    {
      sender: users[4],
      content: "Yes, let's do that. How about in two weeks?",
      createdAt: new Date("2024-06-01T19:35:00Z"),
    },
    {
      sender: users[9],
      content: "Two weeks sounds perfect. I'll check with the venue.",
      createdAt: new Date("2024-06-01T19:40:00Z"),
    },
    {
      sender: users[0],
      content: "Great. Let's make sure we're well-rehearsed by then.",
      createdAt: new Date("2024-06-01T19:45:00Z"),
    },
    {
      sender: users[4],
      content: "Definitely. Let's have a few more practice sessions.",
      createdAt: new Date("2024-06-01T19:50:00Z"),
    },
    {
      sender: users[9],
      content: "I'm in. Let's set up a schedule.",
      createdAt: new Date("2024-06-01T19:55:00Z"),
    },
    {
      sender: users[0],
      content: "How about we practice every other day until the gig?",
      createdAt: new Date("2024-06-01T20:00:00Z"),
    },
    {
      sender: users[4],
      content: "Works for me. Let's stick to that plan.",
      createdAt: new Date("2024-06-01T20:05:00Z"),
    },
    {
      sender: users[9],
      content: "Sounds perfect. See you all at the next practice.",
      createdAt: new Date("2024-06-01T20:10:00Z"),
    },
    {
      sender: users[0],
      content: "Great session today. See you soon!",
      createdAt: new Date("2024-06-01T20:15:00Z"),
    },
    {
      sender: users[4],
      content: "Bye everyone! Have a great evening.",
      createdAt: new Date("2024-06-01T20:20:00Z"),
    },
    {
      sender: users[9],
      content: "Take care, everyone!",
      createdAt: new Date("2024-06-01T20:25:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Hey Cooking Lovers! What should we cook this weekend?",
      createdAt: new Date("2024-06-01T10:00:00Z"),
    },
    {
      sender: users[10],
      content: "How about a themed dinner? Italian night!",
      createdAt: new Date("2024-06-01T10:05:00Z"),
    },
    {
      sender: users[2],
      content: "Italian sounds delicious. I'm in!",
      createdAt: new Date("2024-06-01T10:10:00Z"),
    },
    {
      sender: users[6],
      content: "Perfect! I'll make homemade pasta.",
      createdAt: new Date("2024-06-01T10:15:00Z"),
    },
    {
      sender: users[0],
      content: "Great idea! I'll prepare a Caprese salad.",
      createdAt: new Date("2024-06-01T10:20:00Z"),
    },
    {
      sender: users[10],
      content: "I'll handle the dessert. Tiramisu, anyone?",
      createdAt: new Date("2024-06-01T10:25:00Z"),
    },
    {
      sender: users[2],
      content: "Yum! I'll bring some good wine to pair with the food.",
      createdAt: new Date("2024-06-01T10:30:00Z"),
    },
    {
      sender: users[6],
      content: "This is going to be an amazing dinner!",
      createdAt: new Date("2024-06-01T10:35:00Z"),
    },
    {
      sender: users[0],
      content: "Let's meet at my place at 6 PM on Saturday.",
      createdAt: new Date("2024-06-01T10:40:00Z"),
    },
    {
      sender: users[10],
      content: "Sounds good to me. I'll see you all then.",
      createdAt: new Date("2024-06-01T10:45:00Z"),
    },
    {
      sender: users[2],
      content: "Can't wait! See you all on Saturday.",
      createdAt: new Date("2024-06-01T10:50:00Z"),
    },
    {
      sender: users[6],
      content: "Don't forget to bring your appetite!",
      createdAt: new Date("2024-06-01T10:55:00Z"),
    },
    {
      sender: users[0],
      content: "I hope everyone loves the pasta I'm making.",
      createdAt: new Date("2024-06-02T11:00:00Z"),
    },
    {
      sender: users[10],
      content: "I'm sure it'll be delicious. How's the salad coming along?",
      createdAt: new Date("2024-06-02T11:05:00Z"),
    },
    {
      sender: users[2],
      content: "Just finished making the tiramisu. It looks great!",
      createdAt: new Date("2024-06-02T11:10:00Z"),
    },
    {
      sender: users[6],
      content: "Everything is coming together nicely. See you all soon!",
      createdAt: new Date("2024-06-02T11:15:00Z"),
    },
    {
      sender: users[0],
      content:
        "Just a reminder, if anyone has dietary restrictions, let me know.",
      createdAt: new Date("2024-06-02T11:20:00Z"),
    },
    {
      sender: users[10],
      content: "No restrictions here! Ready for an Italian feast.",
      createdAt: new Date("2024-06-02T11:25:00Z"),
    },
    {
      sender: users[2],
      content: "Same here. See you all soon!",
      createdAt: new Date("2024-06-02T11:30:00Z"),
    },
    {
      sender: users[6],
      content: "Looking forward to it!",
      createdAt: new Date("2024-06-02T11:35:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Hey Photography Crew! Any plans for this weekend?",
      createdAt: new Date("2024-06-01T10:00:00Z"),
    },
    {
      sender: users[7],
      content: "How about a photo walk in the park?",
      createdAt: new Date("2024-06-01T10:15:00Z"),
    },
    {
      sender: users[9],
      content: "Sounds great! I need to test my new lens.",
      createdAt: new Date("2024-06-01T10:30:00Z"),
    },
    {
      sender: users[1],
      content: "I'm in! Let's meet at the park entrance at 9 AM.",
      createdAt: new Date("2024-06-01T10:45:00Z"),
    },
    {
      sender: users[3],
      content: "Perfect. Don't forget to bring extra batteries.",
      createdAt: new Date("2024-06-01T11:00:00Z"),
    },
    {
      sender: users[0],
      content: "Good call. I'll also bring some snacks.",
      createdAt: new Date("2024-06-01T11:15:00Z"),
    },
    {
      sender: users[7],
      content: "I'll bring water bottles. Stay hydrated!",
      createdAt: new Date("2024-06-01T11:30:00Z"),
    },
    {
      sender: users[9],
      content: "Looking forward to it. See you all on Saturday!",
      createdAt: new Date("2024-06-01T11:45:00Z"),
    },
    {
      sender: users[1],
      content: "Hey everyone, ready for our photo walk?",
      createdAt: new Date("2024-06-02T08:30:00Z"),
    },
    {
      sender: users[3],
      content: "Yes! The weather is perfect for photography.",
      createdAt: new Date("2024-06-02T08:45:00Z"),
    },
    {
      sender: users[0],
      content: "I'm on my way. Should be there in 10 minutes.",
      createdAt: new Date("2024-06-02T09:00:00Z"),
    },
    {
      sender: users[7],
      content: "Just arrived. Waiting by the entrance.",
      createdAt: new Date("2024-06-02T09:10:00Z"),
    },
    {
      sender: users[9],
      content: "Let's start with the flower garden.",
      createdAt: new Date("2024-06-02T09:30:00Z"),
    },
    {
      sender: users[1],
      content: "Good idea. The lighting is perfect there.",
      createdAt: new Date("2024-06-02T09:45:00Z"),
    },
    {
      sender: users[3],
      content: "Got some great shots already!",
      createdAt: new Date("2024-06-02T10:00:00Z"),
    },
    {
      sender: users[0],
      content: "This place is amazing. So many photo opportunities.",
      createdAt: new Date("2024-06-02T10:15:00Z"),
    },
    {
      sender: users[7],
      content: "Let's move to the lake next.",
      createdAt: new Date("2024-06-02T10:30:00Z"),
    },
    {
      sender: users[9],
      content: "The reflections on the water should be beautiful.",
      createdAt: new Date("2024-06-02T10:45:00Z"),
    },
    {
      sender: users[1],
      content: "Agreed. Can't wait to capture some stunning shots.",
      createdAt: new Date("2024-06-02T11:00:00Z"),
    },
    {
      sender: users[3],
      content: "I brought my tripod for some long exposures.",
      createdAt: new Date("2024-06-02T11:15:00Z"),
    },
    {
      sender: users[0],
      content: "Great idea. Let's set up and take some group shots too.",
      createdAt: new Date("2024-06-02T11:30:00Z"),
    },
    {
      sender: users[7],
      content: "This was such a fun photo walk. We should do this more often.",
      createdAt: new Date("2024-06-02T11:45:00Z"),
    },
    {
      sender: users[9],
      content:
        "Definitely. Maybe next time we can explore a different location.",
      createdAt: new Date("2024-06-02T12:00:00Z"),
    },
    {
      sender: users[1],
      content: "How about the botanical gardens next weekend?",
      createdAt: new Date("2024-06-03T09:00:00Z"),
    },
    {
      sender: users[3],
      content: "Great idea! I've always wanted to take photos there.",
      createdAt: new Date("2024-06-03T09:15:00Z"),
    },
    {
      sender: users[0],
      content:
        "Count me in. The gardens should be beautiful this time of year.",
      createdAt: new Date("2024-06-03T09:30:00Z"),
    },
    {
      sender: users[7],
      content: "I'm excited! Let's finalize the details later this week.",
      createdAt: new Date("2024-06-03T09:45:00Z"),
    },
    {
      sender: users[9],
      content: "Perfect. I'll create an event for us.",
      createdAt: new Date("2024-06-03T10:00:00Z"),
    },
    {
      sender: users[1],
      content: "Thanks! Can't wait for our next adventure.",
      createdAt: new Date("2024-06-03T10:15:00Z"),
    },
    {
      sender: users[3],
      content: "This crew rocks. See you all soon!",
      createdAt: new Date("2024-06-03T10:30:00Z"),
    },
    {
      sender: users[0],
      content: "Take care, everyone! Happy shooting!",
      createdAt: new Date("2024-06-03T10:45:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Hey Movie Buffs! What movie should we watch this weekend?",
      createdAt: new Date("2024-06-01T18:00:00Z"),
    },
    {
      sender: users[5],
      content: "How about the new sci-fi movie that just came out?",
      createdAt: new Date("2024-06-01T18:15:00Z"),
    },
    {
      sender: users[8],
      content: "I've heard great things about it. I'm in!",
      createdAt: new Date("2024-06-01T18:30:00Z"),
    },
    {
      sender: users[4],
      content: "Sounds good to me. Should we watch it at my place?",
      createdAt: new Date("2024-06-01T18:45:00Z"),
    },
    {
      sender: users[0],
      content: "Perfect! I'll bring the popcorn.",
      createdAt: new Date("2024-06-01T19:00:00Z"),
    },
    {
      sender: users[5],
      content: "I'll bring some drinks. What time should we meet?",
      createdAt: new Date("2024-06-01T19:15:00Z"),
    },
    {
      sender: users[8],
      content: "How about 7 PM? Does that work for everyone?",
      createdAt: new Date("2024-06-01T19:30:00Z"),
    },
    {
      sender: users[4],
      content: "7 PM works for me. Looking forward to it!",
      createdAt: new Date("2024-06-01T19:45:00Z"),
    },
    {
      sender: users[0],
      content: "Great! See you all at 7 PM.",
      createdAt: new Date("2024-06-01T20:00:00Z"),
    },
    {
      sender: users[5],
      content: "Can't wait! See you all then.",
      createdAt: new Date("2024-06-01T20:15:00Z"),
    },
    {
      sender: users[8],
      content: "Just got the drinks. Ready for tonight!",
      createdAt: new Date("2024-06-02T16:00:00Z"),
    },
    {
      sender: users[4],
      content: "Popcorn is ready too. This is going to be fun.",
      createdAt: new Date("2024-06-02T16:15:00Z"),
    },
    {
      sender: users[0],
      content: "I'm on my way. See you soon!",
      createdAt: new Date("2024-06-02T16:30:00Z"),
    },
    {
      sender: users[5],
      content: "Everything is set up here. Just waiting for you all.",
      createdAt: new Date("2024-06-02T16:45:00Z"),
    },
    {
      sender: users[8],
      content: "Arriving in 10 minutes. Ready for movie night!",
      createdAt: new Date("2024-06-02T17:00:00Z"),
    },
    {
      sender: users[4],
      content: "Just arrived. Let's get this started!",
      createdAt: new Date("2024-06-02T17:15:00Z"),
    },
    {
      sender: users[0],
      content: "Awesome! Let's watch the movie.",
      createdAt: new Date("2024-06-02T17:30:00Z"),
    },
    {
      sender: users[5],
      content: "This movie is fantastic!",
      createdAt: new Date("2024-06-02T18:00:00Z"),
    },
    {
      sender: users[8],
      content: "Agreed. The special effects are amazing.",
      createdAt: new Date("2024-06-02T18:15:00Z"),
    },
    {
      sender: users[4],
      content: "Such a great plot twist!",
      createdAt: new Date("2024-06-02T18:30:00Z"),
    },
    {
      sender: users[0],
      content: "That was a great movie. We should do this more often.",
      createdAt: new Date("2024-06-02T19:00:00Z"),
    },
    {
      sender: users[5],
      content: "Definitely. Let's plan for next weekend.",
      createdAt: new Date("2024-06-02T19:15:00Z"),
    },
    {
      sender: users[8],
      content: "I'm in. How about a classic next time?",
      createdAt: new Date("2024-06-02T19:30:00Z"),
    },
    {
      sender: users[4],
      content: "Sounds good. Maybe an old Hitchcock film?",
      createdAt: new Date("2024-06-02T19:45:00Z"),
    },
    {
      sender: users[0],
      content: "I love Hitchcock! Let's do it.",
      createdAt: new Date("2024-06-02T20:00:00Z"),
    },
    {
      sender: users[5],
      content: "Awesome. Let's meet again at the same time.",
      createdAt: new Date("2024-06-03T18:00:00Z"),
    },
    {
      sender: users[8],
      content: "Great. See you all next weekend!",
      createdAt: new Date("2024-06-03T18:15:00Z"),
    },
    {
      sender: users[4],
      content: "Looking forward to it. Bye everyone!",
      createdAt: new Date("2024-06-03T18:30:00Z"),
    },
    {
      sender: users[0],
      content: "See you all soon!",
      createdAt: new Date("2024-06-03T18:45:00Z"),
    },
    {
      sender: users[5],
      content: "Take care!",
      createdAt: new Date("2024-06-03T19:00:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Hey Tech Geeks! Did you see the latest release from Apple?",
      createdAt: new Date("2024-06-01T09:00:00Z"),
    },
    {
      sender: users[3],
      content:
        "Yes! The new features look amazing. Especially the AI enhancements.",
      createdAt: new Date("2024-06-01T09:15:00Z"),
    },
    {
      sender: users[10],
      content:
        "I'm excited to try out the new hardware. The specs are impressive.",
      createdAt: new Date("2024-06-01T09:30:00Z"),
    },
    {
      sender: users[0],
      content:
        "Agreed. Do you think it's worth upgrading from last year's model?",
      createdAt: new Date("2024-06-01T09:45:00Z"),
    },
    {
      sender: users[3],
      content:
        "Depends on what you're looking for. The performance boost seems significant.",
      createdAt: new Date("2024-06-01T10:00:00Z"),
    },
    {
      sender: users[10],
      content: "I think it might be worth it for the new camera system alone.",
      createdAt: new Date("2024-06-01T10:15:00Z"),
    },
    {
      sender: users[0],
      content: "True. The camera upgrades are always tempting.",
      createdAt: new Date("2024-06-01T10:30:00Z"),
    },
    {
      sender: users[3],
      content:
        "What about software? Any new apps or features you're excited about?",
      createdAt: new Date("2024-06-01T10:45:00Z"),
    },
    {
      sender: users[10],
      content: "The new AI-driven photo editing app looks promising.",
      createdAt: new Date("2024-06-01T11:00:00Z"),
    },
    {
      sender: users[0],
      content: "Yes, I saw the demo. It seems to make editing so much easier.",
      createdAt: new Date("2024-06-01T11:15:00Z"),
    },
    {
      sender: users[3],
      content:
        "Can't wait to test it out. Do you think it'll be available on older models?",
      createdAt: new Date("2024-06-01T11:30:00Z"),
    },
    {
      sender: users[10],
      content: "Most likely, but it might run better on the new hardware.",
      createdAt: new Date("2024-06-01T11:45:00Z"),
    },
    {
      sender: users[0],
      content: "Let's plan a meetup to try out the new features together.",
      createdAt: new Date("2024-06-01T12:00:00Z"),
    },
    {
      sender: users[3],
      content: "Great idea! How about this Saturday at my place?",
      createdAt: new Date("2024-06-01T12:15:00Z"),
    },
    {
      sender: users[10],
      content: "Works for me. I'll bring some snacks.",
      createdAt: new Date("2024-06-01T12:30:00Z"),
    },
    {
      sender: users[0],
      content: "I'll bring the new device. See you all on Saturday!",
      createdAt: new Date("2024-06-01T12:45:00Z"),
    },
    {
      sender: users[3],
      content: "Looking forward to it. See you then!",
      createdAt: new Date("2024-06-01T13:00:00Z"),
    },
    {
      sender: users[10],
      content: "Can't wait! It's going to be fun.",
      createdAt: new Date("2024-06-01T13:15:00Z"),
    },
    {
      sender: users[0],
      content: "Don't forget to bring your gadgets for a hands-on session.",
      createdAt: new Date("2024-06-01T13:30:00Z"),
    },
    {
      sender: users[3],
      content: "Will do! See you soon, Tech Geeks!",
      createdAt: new Date("2024-06-01T13:45:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Hey Fitness Freaks! Ready for our workout session tomorrow?",
      createdAt: new Date("2024-06-01T07:00:00Z"),
    },
    {
      sender: users[6],
      content: "Absolutely! What's the plan for tomorrow?",
      createdAt: new Date("2024-06-01T07:15:00Z"),
    },
    {
      sender: users[2],
      content: "I'm in! Should we start with some cardio?",
      createdAt: new Date("2024-06-01T07:30:00Z"),
    },
    {
      sender: users[9],
      content: "Cardio sounds great. How about a 5k run?",
      createdAt: new Date("2024-06-01T07:45:00Z"),
    },
    {
      sender: users[0],
      content: "I'm up for it! Let's meet at the park at 6 AM.",
      createdAt: new Date("2024-06-01T08:00:00Z"),
    },
    {
      sender: users[6],
      content: "Perfect. Don't forget to bring water and a towel.",
      createdAt: new Date("2024-06-01T08:15:00Z"),
    },
    {
      sender: users[2],
      content: "Got it! See you all tomorrow morning.",
      createdAt: new Date("2024-06-01T08:30:00Z"),
    },
    {
      sender: users[9],
      content: "Looking forward to it. Let's smash our goals!",
      createdAt: new Date("2024-06-01T08:45:00Z"),
    },
    {
      sender: users[0],
      content: "Morning everyone! Ready for our run?",
      createdAt: new Date("2024-06-02T05:45:00Z"),
    },
    {
      sender: users[6],
      content: "Absolutely! Let's get warmed up.",
      createdAt: new Date("2024-06-02T06:00:00Z"),
    },
    {
      sender: users[2],
      content: "Warm-up done. Ready to start the run?",
      createdAt: new Date("2024-06-02T06:15:00Z"),
    },
    {
      sender: users[9],
      content: "Let's go! 5k, here we come!",
      createdAt: new Date("2024-06-02T06:30:00Z"),
    },
    {
      sender: users[0],
      content: "Great run, everyone! How about some strength training next?",
      createdAt: new Date("2024-06-02T07:00:00Z"),
    },
    {
      sender: users[6],
      content: "Sounds good. Let's meet at the gym at 5 PM.",
      createdAt: new Date("2024-06-02T07:15:00Z"),
    },
    {
      sender: users[2],
      content: "I'll bring the protein shakes!",
      createdAt: new Date("2024-06-02T07:30:00Z"),
    },
    {
      sender: users[9],
      content: "Perfect. See you all at the gym.",
      createdAt: new Date("2024-06-02T07:45:00Z"),
    },
    {
      sender: users[0],
      content: "Ready for some strength training?",
      createdAt: new Date("2024-06-02T16:45:00Z"),
    },
    {
      sender: users[6],
      content: "Let's do it! I'll start with some bench presses.",
      createdAt: new Date("2024-06-02T17:00:00Z"),
    },
    {
      sender: users[2],
      content: "I'm hitting the squats first.",
      createdAt: new Date("2024-06-02T17:15:00Z"),
    },
    {
      sender: users[9],
      content: "Great session today! Let's keep pushing ourselves.",
      createdAt: new Date("2024-06-02T17:30:00Z"),
    },
  ],
  [
    {
      sender: users[0],
      content: "Hey Travel Buddies! Any ideas for our next trip?",
      createdAt: new Date("2024-06-01T10:00:00Z"),
    },
    {
      sender: users[7],
      content: "How about a trip to the mountains? We could go hiking.",
      createdAt: new Date("2024-06-01T10:15:00Z"),
    },
    {
      sender: users[10],
      content: "I love that idea! Fresh air and beautiful scenery.",
      createdAt: new Date("2024-06-01T10:30:00Z"),
    },
    {
      sender: users[1],
      content: "Count me in! When should we go?",
      createdAt: new Date("2024-06-01T10:45:00Z"),
    },
    {
      sender: users[8],
      content: "How about next weekend? We can leave Friday evening.",
      createdAt: new Date("2024-06-01T11:00:00Z"),
    },
    {
      sender: users[0],
      content: "Next weekend works for me. I'll check for good hiking trails.",
      createdAt: new Date("2024-06-01T11:15:00Z"),
    },
    {
      sender: users[7],
      content: "I'll look for a nice cabin to stay in.",
      createdAt: new Date("2024-06-01T11:30:00Z"),
    },
    {
      sender: users[10],
      content: "I'll take care of the food and snacks.",
      createdAt: new Date("2024-06-01T11:45:00Z"),
    },
    {
      sender: users[1],
      content: "Perfect! Let's finalize the details by mid-week.",
      createdAt: new Date("2024-06-01T12:00:00Z"),
    },
    {
      sender: users[8],
      content: "Hey guys, found some great trails in the area!",
      createdAt: new Date("2024-06-02T10:00:00Z"),
    },
    {
      sender: users[0],
      content: "Awesome! I booked a cozy cabin for us.",
      createdAt: new Date("2024-06-02T10:15:00Z"),
    },
    {
      sender: users[7],
      content: "I've got the food sorted. Any dietary preferences?",
      createdAt: new Date("2024-06-02T10:30:00Z"),
    },
    {
      sender: users[10],
      content: "Nothing specific from my side. Just excited for the trip!",
      createdAt: new Date("2024-06-02T10:45:00Z"),
    },
    {
      sender: users[1],
      content: "Same here. Can't wait to hit the trails.",
      createdAt: new Date("2024-06-03T10:00:00Z"),
    },
    {
      sender: users[8],
      content: "Should we plan any activities besides hiking?",
      createdAt: new Date("2024-06-03T10:15:00Z"),
    },
    {
      sender: users[0],
      content: "Maybe a bonfire at night? I can bring marshmallows.",
      createdAt: new Date("2024-06-03T10:30:00Z"),
    },
    {
      sender: users[7],
      content: "A bonfire sounds fun. Let's do it.",
      createdAt: new Date("2024-06-03T10:45:00Z"),
    },
    {
      sender: users[10],
      content: "I've got a portable speaker for some music too.",
      createdAt: new Date("2024-06-04T10:00:00Z"),
    },
    {
      sender: users[1],
      content: "Nice! This trip is going to be epic.",
      createdAt: new Date("2024-06-04T10:15:00Z"),
    },
    {
      sender: users[8],
      content: "Just a few more days! So excited.",
      createdAt: new Date("2024-06-04T10:30:00Z"),
    },
    {
      sender: users[0],
      content: "Don't forget to pack warm clothes. It might get chilly.",
      createdAt: new Date("2024-06-05T10:00:00Z"),
    },
    {
      sender: users[7],
      content: "Good call. I'll bring an extra blanket.",
      createdAt: new Date("2024-06-05T10:15:00Z"),
    },
    {
      sender: users[10],
      content: "Packing my bags tonight. Let's do this!",
      createdAt: new Date("2024-06-05T10:30:00Z"),
    },
    {
      sender: users[1],
      content: "Same here. Can't wait to get on the road.",
      createdAt: new Date("2024-06-05T10:45:00Z"),
    },
    {
      sender: users[8],
      content: "Ready for the trip of a lifetime!",
      createdAt: new Date("2024-06-06T10:00:00Z"),
    },
    {
      sender: users[0],
      content: "Just checked the weather. It's going to be perfect.",
      createdAt: new Date("2024-06-06T10:15:00Z"),
    },
    {
      sender: users[7],
      content: "Awesome! Let's meet at my place and carpool.",
      createdAt: new Date("2024-06-06T10:30:00Z"),
    },
    {
      sender: users[10],
      content: "Sounds good. See you all soon!",
      createdAt: new Date("2024-06-06T10:45:00Z"),
    },
    {
      sender: users[1],
      content: "Just finished packing. Ready for adventure!",
      createdAt: new Date("2024-06-06T11:00:00Z"),
    },
    {
      sender: users[8],
      content: "Me too. This is going to be so much fun.",
      createdAt: new Date("2024-06-06T11:15:00Z"),
    },
    {
      sender: users[0],
      content: "Can't wait to make some great memories!",
      createdAt: new Date("2024-06-06T11:30:00Z"),
    },
  ],
];

export default messages;
